...
class HtmlView extends BaseHtmlView
{
    ...
    protected function prepareDocument(): void
    {
        ...
        // the tag view. The root node is excluded for the reason the canonical
        // route below excludes it: it stands for "no category", so its own
        // metadata would apply to every top-level listing.
        $ownCategory = $this->parent && $this->parent->id > 1;

        if ($ownCategory && !empty($this->parent->metadesc)) {
        ...
        // Set meta keywords
        if ($ownCategory && !empty($this->parent->metakey)) {
