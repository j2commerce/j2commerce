                        ...
                        <input type="radio"
                            ...
                            <?php } else { ?>
                                <?php $ovPriceSuffix = (($ov['price_from'] ?? null) !== null && $params->get('product_option_price', 1))
                                    ...
                                    : ''; ?>
                                <label class="btn btn-sm btn-outline-secondary form-check-label fs-xs" for="<?php echo $optionValueInputId; ?>" data-label="<?php echo $esc(Text::_($ov['optionvalue_name']) . $ovPriceSuffix); ?>">
                    ...
                    <div class="j2commerce-color-options d-flex flex-wrap gap-2" data-binded-label="#colorOption<?php echo $optionId; ?>">
                        <?php foreach ($option['optionvalue'] as $ov) : ?>
                            ...
                            <?php $ovPriceSuffix = (($ov['price_from'] ?? null) !== null && $params->get('product_option_price', 1))
                                ? ' (' . Text::_('COM_J2COMMERCE_FROM_PRICE') . ' ' . strip_tags(J2CommerceHelper::product()->displayPrice($ov['price_from'], $product, $params, 'products.view.option')) . ')'
                                : ''; ?>
                            <label for="<?php echo $optionValueInputId; ?>" class="btn btn-color fs-xl" data-label="<?php echo $esc(Text::_($ov['optionvalue_name']) . $ovPriceSuffix); ?>" style="color:<?php echo $esc($ov['optionvalue_image']); ?>;">
                                <span class="visually-hidden"><?php echo $esc(Text::_($ov['optionvalue_name'])); ?></span>
                            </label>
                        <?php endforeach; ?>
                    </div>
