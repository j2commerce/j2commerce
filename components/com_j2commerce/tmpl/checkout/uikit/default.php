...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function fetchStep(task, stepId) {
            ...
            if (!response.ok) {
                return response.text().then(function(text) {
                    ...
                    if (response.status === 403) {
                        var expired = J2CommerceDom.el('div', {class: 'uk-alert uk-alert-warning', 'uk-alert': '', role: 'alert'});
                        ...
                        var failed = J2CommerceDom.el('div', {class: 'uk-alert uk-alert-danger', 'uk-alert': '', role: 'alert'}, <?php echo $serverErrorJs; ?>.replace('%s', response.status));
                        if (detail) {
        ...
        .catch(function(error) {
            ...
            content.setAttribute('aria-busy', 'false');
            content.replaceChildren(J2CommerceDom.el('div', {class: 'uk-alert uk-alert-danger', 'uk-alert': '', role: 'alert'}, <?php echo $networkErrorJs; ?>));
        });
    }
