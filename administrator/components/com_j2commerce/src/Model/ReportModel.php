...
class ReportModel extends AdminModel
{
    ...
    public function save($data)
    {
        ...
        if (
            ...
            || !$table->load($pk)
            || $table->type !== 'plugin'
            || $table->folder !== 'j2commerce'
            || !str_starts_with((string) $table->element, 'report_')
        ) {
    ...
    public function publish(&$pks, $value = 1)
    {
        ...
        // Access checks.
        foreach ($pks as $i => $pk) {
            // Same confinement getItem() and save() declare: this model owns the j2commerce
            // report plugin rows on the shared #__extensions table, and nothing else on it.
            if (
                !$table->load($pk)
                || $table->type !== 'plugin'
                || $table->folder !== 'j2commerce'
