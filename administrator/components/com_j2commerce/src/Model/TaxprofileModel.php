...
class TaxprofileModel extends AdminModel
{
    ...
    /** Children before the parent, so a mid-cascade failure leaves a re-deletable tax profile. */
        ...
        foreach ($this->deletableKeys($pks) as $pk) {
            ...
            $db->setQuery(
                $db->getQuery(true)
                    ...
                    ->where($db->quoteName('taxprofile_id') . ' = :taxprofileId')
                    ->bind(':taxprofileId', $pk, ParameterType::INTEGER)
            )->execute();
        }
